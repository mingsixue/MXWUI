...
<img src="../imgs/components/Mask.PNG" alt="Mask" width="280" />
...
|opacity|Number||0-1之间|`0.6`|蒙层透明度|
|zIndex|Number||数字|`520`|蒙层层级|
|lockScroll|Boolean||`true`、`false`|`true`|是否锁定页面滚动，防止穿透|
...
## 其他说明
弹窗显示时，Mask 默认会锁定页面滚动（`lockScroll`，默认 `true`），避免滑动弹层内容时带动蒙层下方的页面。多层弹窗通过引用计数正确嵌套锁定/解锁。

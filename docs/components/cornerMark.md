...
<img src="../imgs/components/CornerMark.PNG" alt="CornerMark" width="280" />
<img src="../imgs/components/CornerMark2.PNG" alt="CornerMark 2" width="280" />

## 扫码查看

![扫码查看](../imgs/cornerMark_qrcode.png)
...
{
    "usingComponents": {
        "mx-corner-mark": "/components/mxwui/corner-mark/index"
    }
}
```
...
- 文本角标
```html
<mx-corner-mark type="text" />
```

- 图片角标
```html
<mx-corner-mark type="image" />
```
...
```html
<mx-corner-mark textColor="#1677FF" />
```

### # 文案字号 textSize
属性：`textSize`，默认 `24`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark textSize="32" />
```

### # 角标背景色 bgColor
属性：`bgColor`，默认 `#CA0E2D`，支持任何合法的颜色值。
```html
<mx-corner-mark bgColor="#1677FF" />
```

### # 角标圆角 radius
属性：`radius`，默认 `16`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark radius="32" />
```

### # 距离顶部 top
属性：`top`，默认 `0`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark top="24" />
```

### # 距离左侧 left
属性：`left`，默认 `0`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark left="24" />
```

### # 距离右侧 right
属性：`right`，默认 `0`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark right="24" />
```

### # 距离底部 bottom
属性：`bottom`，默认 `0`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark bottom="24" />
```

### # 角标自定义图片 imgUrl
属性：`imgUrl`，自定义图片地址。
```html
<mx-corner-mark imgUrl="图片地址" />
```

### # 角标图片宽度 imagWidth
属性：`imagWidth`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark imagWidth="24" />
```

### # 角标图片高度 imgHeight
属性：`imgHeight`，默认 `48`，数字，不带单位，默认使用rpx。
```html
<mx-corner-mark imgHeight="24" />
```

<!-- ## 参数示意图
![组件参数分解示意图](../imgs/cornerMark_params.png) -->

...
<img src="../imgs/components/Datetime.PNG" alt="Datetime" width="280" />
<img src="../imgs/components/Datetime2.PNG" alt="Datetime 2" width="280" />
<img src="../imgs/components/Datetime3.PNG" alt="Datetime 3" width="280" />
<img src="../imgs/components/Datetime4.PNG" alt="Datetime 4" width="280" />
<img src="../imgs/components/Datetime5.PNG" alt="Datetime 5" width="280" />
...
### # 最小日期 minDate
属性：`minDate`，限制可选的最小日期时间，格式同 `value`（如 `YYYY-MM-DD HH:mm`）。
```html
<mx-datetime minDate="2024-01-01 00:00" />
```

### # 最大日期 maxDate
属性：`maxDate`，限制可选的最大日期时间，格式同 `value`（如 `YYYY-MM-DD HH:mm`）。
```html
<mx-datetime maxDate="2025-12-31 23:59" />
```

### # 可选范围限制
同时设置 `minDate`、`maxDate`，滚动选择时年月日時分秒会联动裁剪。
```html
<mx-datetime minDate="2023-04-01 19:30" maxDate="2024-05-21 18:35" />
```

### # 最小年份限制 minYearLimit
属性：`minYearLimit`，默认 `1970`，数字。未设置 `minDate` 时生效。
```html
<mx-datetime minYearLimit="2000" />
```

### # 最大年份限制 maxYearLimit
属性：`maxYearLimit`，默认在当前年+20年。未设置 `maxDate` 时生效。
```html
...
|minDate|String||YYYY-MM-DD HH:mm||最小日期限制|
|maxDate|String||YYYY-MM-DD HH:mm||最大日期限制|

...
function ensureLunisolar() {
    if (lunisolarInstance) {
        return lunisolarInstance;
    }
    // 按需加载农历库，避免日历组件初始化即拉入 ~44KB 逻辑
    // eslint-disable-next-line global-require
    let lunisolar = require('./lib/lunisolar');
    lunisolar = lunisolar.default || lunisolar;
    // eslint-disable-next-line global-require
    let festivals = require('./lib/festivals-short.zh-cn');
    festivals = festivals.default || festivals;
...
Component({
    ...
    methods: {
        ...
        dealLunar(date) {
            ...
            // 按需初始化农历
            const lunisolar = ensureLunisolar();
